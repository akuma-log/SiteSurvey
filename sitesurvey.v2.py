...
class HighlightRenderer(DefaultTableCellRenderer):
    ...
    def getTableCellRendererComponent(self, table, value, isSelected, hasFocus, row, column):
        ...
        # Get the actual model row and request number
        model_row = table.convertRowIndexToModel(row)
        req_no = table.getModel().getValueAt(model_row, 0)
        ...
        if is_dark:
            ...
            highlight_fg = Color.BLACK  # Text color when highlighted in dark theme
        else:
            default_fg = Color.BLACK
            default_bg = self.light_bg
            highlight_fg = Color.WHITE  # Text color when highlighted in light theme
...
class BurpExtender(IBurpExtender, IHttpListener, ITab):
    ...
    def _init_ui_components(self):
        ...
        # Restore Excel-like UI
        self.log_table.setShowGrid(True)  # Show grid lines
        self.log_table.setGridColor(Color.LIGHT_GRAY)
        self.log_table.setSelectionBackground(Color(200, 200, 255))  # Light blue selection
        self.log_table.setSelectionForeground(Color.BLACK)
    ...
    def _highlight_selected(self, color):
        ...
        for row in selected_rows:
            req_no = int(self.log_model.getValueAt(row, 0))
            self.highlight_renderer.setHighlightColor(req_no, color)
        
        self.log_table.repaint()

    def _setup_control_buttons(self):
        self.copy_button = JButton("Select Columns to Copy", actionPerformed=self._show_copy_dialog)
    ...
    def _delete_selected(self, event=None):
        ...
        if confirm == JOptionPane.YES_OPTION:
            # Get request numbers of selected rows
            selected_numbers = [int(self.log_model.getValueAt(row, 0)) for row in selected_rows]
            
            # Create mapping for highlight migration
            old_to_new_map = {}
            new_number = 1
            
            # Build new requests list with updated numbers
            new_requests = []
            for req in self.requests:
                if req['number'] not in selected_numbers:
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        ...
        if messageIsRequest:
            ...
            entry = {
                ...
                'params': len(analyzed.getParameters()),
                'status': "Pending",
                'length': 0,
    ...
    def _get_parameters_count(self, analyzed_request):
        ...
        return len(analyzed_request.getParameters())

    def _show_copy_dialog(self, event):
        dialog = JDialog()
        dialog.setTitle("Select Columns to Copy")
        dialog.setSize(400, 300)
        dialog.setLayout(BorderLayout())

        # Main panel with GridLayout for checkboxes
        main_panel = JPanel(GridLayout(0, 1, 5, 5))  # Single column, with spacing
        scroll_pane = JScrollPane(main_panel)
        
        # Get all column names
        column_names = [self.log_model.getColumnName(i) for i in range(self.log_model.getColumnCount())]
        ...
        # Button panel at bottom (similar to other dialogs)
        button_panel = JPanel(FlowLayout(FlowLayout.CENTER))
        
        # Select All button
        select_all = JButton("Select All")
        select_all.addActionListener(lambda e: [cb.setSelected(True) for cb in self.column_checkboxes])
        button_panel.add(select_all)
        
        # Deselect All button
        deselect_all = JButton("Deselect All")
        deselect_all.addActionListener(lambda e: [cb.setSelected(False) for cb in self.column_checkboxes])
        button_panel.add(deselect_all)
        
        # Copy button
        copy_btn = JButton("Copy")
        copy_btn.addActionListener(lambda e: self._copy_selected_columns(dialog))
    ...
    def _copy_selected_columns(self, dialog):
        # Get selected column indices
        selected_indices = []
        selected_names = []
        for i in range(self.log_model.getColumnCount()):
            cb = self.column_checkboxes[i]
            if cb.isSelected():
                selected_indices.append(i)
                selected_names.append(self.log_model.getColumnName(i))
        ...
        data = []
        # Add headers
        data.append("\t".join(selected_names))
        # Add rows
        for row in range(self.log_model.getRowCount()):
            row_data = []
        ...
        try:
            ...
            JOptionPane.showMessageDialog(dialog,
                "Copied {} columns to clipboard".format(len(selected_indices)),
                "Copy Successful",
                JOptionPane.INFORMATION_MESSAGE)
        except Exception as e:
            JOptionPane.showMessageDialog(dialog,
    ...
    def getUiComponent(self):
        ...
        control_panel.add(self.copy_button)  # Changed from export_button
    ...
    def _apply_filters(self, dialog):
        ...
        self._include_extensions = []
        if include_text:
            self._include_extensions = [ext.strip().lower() for ext in include_text.split(",") if ext.strip()]
            # Ensure extensions start with dot
            self._include_extensions = [ext if ext.startswith('.') else '.'+ext for ext in self._include_extensions]

        # Process exclude extensions
        self._exclude_extensions = []
        if exclude_text:
            self._exclude_extensions = [ext.strip().lower() for ext in exclude_text.split(",") if ext.strip()]
            # Ensure extensions start with dot
            self._exclude_extensions = [ext if ext.startswith('.') else '.'+ext for ext in self._exclude_extensions]

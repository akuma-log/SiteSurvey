...
class BurpExtender(IBurpExtender, IHttpListener, ITab):  # Remove IMessageEditorController
    ...
    def _export_to_excel(self, event):
        ...
        default_filename = "site_survey_export_{}.csv".format(timestamp)
    ...
    def _copy_selected(self, event=None):
        ...
        try:
            ...
            for row in selected_rows:
                ...
                clipboard_data.append(u"\t".join(row_data))
            ...
            full_text = u"\n".join(clipboard_data)
            clipboard.setContents(StringSelection(full_text), None)
        ...
        try:
            ...
            for req_index, req in display_requests:
                ...
                row_data = [
                    ...
                    safe_get_value(req.get('YOUR_NEW_FIELD', "")) 
                ]
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        ...
        if messageIsRequest:
            ...
            # Create the entry - PASS messageInfo to _get_parameters_count
            entry = {
                ...
                'params': self._get_parameters_count(analyzed, messageInfo),  # Pass messageInfo here
                'status': "Pending",
                'length': 0,
            ...
            # Process response... (keep your existing code)
    ...
    def _get_parameters_count(self, analyzed_request, messageInfo=None):
        """Count URL query parameters and request body parameters"""
        try:
            param_count = 0
            
            # Count URL query parameters (works for ALL HTTP methods)
            url = analyzed_request.getUrl().toString()
            
            # Extract and count URL parameters
            if '?' in url:
                query_string = url.split('?', 1)[1].split('#')[0]
                if query_string and query_string.strip():
                    # Count unique parameter names
                    import urllib
                    try:
                        parsed = urllib.parse_qs(query_string, keep_blank_values=True)
                        param_count += len(parsed)
                    except:
                        # Fallback: simple count by '&'
                        if '&' in query_string:
                            seen_params = set()
                            ...
                            param_count += len(seen_params)
            
            # For POST/PUT/PATCH, also check body parameters
            method = analyzed_request.getMethod().upper()
            
            if method in ["POST", "PUT", "PATCH"] and messageInfo:
                # Get request body from messageInfo, not analyzed_request
                request_bytes = messageInfo.getRequest()
                
                if request_bytes:
                    # Get body offset from analyzed_request
                    ...
                    if body_offset < len(request_bytes):
                        ...
                        if body and body.strip():  # Only count if body has content
                            # Check Content-Type
                            ...
                            if "application/x-www-form-urlencoded" in content_type.lower():
                                ...
                                if '&' in body:
                                    ...
                                    for param in body.split('&'):
                                        if '=' in param:
                                            name = param.split('=', 1)[0]
                                            seen_body_params.add(name)
                                    param_count += len(seen_body_params)
                            ...
                            # Multipart/form-data (file uploads)
                                ...
                                # Count form fields
                                import re
                                name_matches = re.findall(r'name="([^"]+)"', body)
                                param_count += len(set(name_matches))
            
            return param_count
            
        except Exception as e:
            # If anything goes wrong, return 0 to keep the tool running
            print "[ERROR] _get_parameters_count failed: {}".format(str(e))
            return 0
    ...
    def getUiComponent(self):
        ...
        footer_panel.add(JLabel("Created by Hossain", foreground=Color.GRAY))
    ...
    def _extract_button_from_response(self, response_bytes, request_url=None):
        """Extract button name from HTML response - improved version"""
        ...
        # Fallback to the simpler method (existing code)
        try:
    ...
    def _extract_button_simple(self, response_bytes, request_url):
        """Robust button extraction with better URL matching"""
        ...
        try:
            url_obj = URL(request_url)
            request_host = url_obj.getHost()
            request_path = url_obj.getPath()
            request_full_path = request_path
        except:
            # Fallback parsing
            request_host = ""
            request_path = ""
            match = re.match(r'https?://([^/]+)(/.*)', request_url)
            if match:
                request_host = match.group(1)
        ...
        # METHOD 1: Look for anchor tags with matching hrefs
        anchor_pattern = r'<a\s+[^>]*href\s*=\s*["\']([^"\']+)["\'][^>]*>([^<]+)</a>'
        ...
        for href, text in all_anchors:
            ...
            # Clean the href
            decoded_href = decoded_href.split('?')[0].split('#')[0].rstrip('/')
            
            # Check multiple matching strategies
            
            # Strategy 1: Exact match with full path
            if decoded_href == request_path:
                return self._clean_and_extract_text(text)
            
            # Strategy 2: Match after domain (for full URLs in href)
            if decoded_href.startswith('http'):
                try:
                    href_url = URL(decoded_href)
                    href_path = href_url.getPath().rstrip('/')
                    if href_path == request_path:
                        return self._clean_and_extract_text(text)
                except:
                    pass
            
            # Strategy 3: Relative path match
            # Check if href is a relative path that matches our request path
            if not decoded_href.startswith('http'):
                # Try with leading slash
                if not decoded_href.startswith('/'):
                    decoded_href = '/' + decoded_href
                
                # Check if request path ends with the href path
                if request_path.endswith(decoded_href):
                    return self._clean_and_extract_text(text)
                
                # Check the last segment match
                if decoded_href.endswith(request_path.split('/')[-1]):
                    return self._clean_and_extract_text(text)
        
        # METHOD 2: Look specifically in navigation menus
        nav_patterns = [
            (r'<ul[^>]*class=["\'][^"\']*pulldown[^"\']*["\'][^>]*>(.*?)</ul>', 'pulldown'),
            (r'<ul[^>]*class=["\'][^"\']*nav[^"\']*["\'][^>]*>(.*?)</ul>', 'nav'),
            (r'<nav[^>]*>(.*?)</nav>', 'nav-tag'),
            (r'<ul[^>]*>(.*?)</ul>', 'ul'),
            (r'<ol[^>]*>(.*?)</ol>', 'ol'),
        ]
        ...
        for pattern, pattern_type in nav_patterns:
            ...
            for nav_content in nav_matches:
                # Search within this navigation section
                for href, text in re.findall(anchor_pattern, nav_content, re.IGNORECASE | re.DOTALL):
                    try:
                        decoded_href = urllib.unquote(href)
                    except:
                        decoded_href = href
                    ...
                    if not decoded_href.startswith('http') and not decoded_href.startswith('/'):
                        decoded_href = '/' + decoded_href
                    ...
                    # Match by filename
                    request_filename = request_path.split('/')[-1] if '/' in request_path else request_path
                    href_filename = decoded_href.split('/')[-1] if '/' in decoded_href else decoded_href
                    
                    if request_filename and href_filename and request_filename == href_filename:
                        return self._clean_and_extract_text(text)
        
        # METHOD 3: Look for the link in context of specific sections
        # Try to find sections that contain the current path
        section_patterns = [
        ...
        for pattern in section_patterns:
            section_matches = re.findall(pattern, response_str, re.IGNORECASE | re.DOTALL)
            
            for section_content in section_matches:
                for href, text in re.findall(anchor_pattern, section_content, re.IGNORECASE | re.DOTALL):
                    try:
                        decoded_href = urllib.unquote(href)
                    except:
                        decoded_href = href
                    ...
                    request_filename = request_path.split('/')[-1] if '/' in request_path else request_path
                    if request_filename and request_filename in decoded_href:
                        return self._clean_and_extract_text(text)
        
        # METHOD 4: If no exact match, try to find the most relevant link
        # Look for links that contain the last part of the path
        last_path_segment = request_path.split('/')[-1] if '/' in request_path else request_path
        
        if last_path_segment and len(last_path_segment) > 1:
            for href, text in all_anchors:
                if last_path_segment in href:
                    cleaned_text = self._clean_and_extract_text(text)
                    if cleaned_text:
                        return cleaned_text
        
        return None

    def _clean_and_extract_text(self, text):
        """Helper to clean and extract text from HTML"""
        import re
            ...
            if row < len(self._display_to_request_map):
                ...
                if actual_index < len(self.requests):
                    ...
                    if response_bytes and request_url:
                        ...
                        if button_name:
                            self.log_model.setValueAt(button_name, row, 3)  
                            req['button_name'] = button_name
                            extracted_count += 1

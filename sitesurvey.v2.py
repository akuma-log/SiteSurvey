...
class BurpExtender(IBurpExtender, IHttpListener, ITab):
    ...
    def _view_in_repeater(self):
        ...
        for req in self.requests:
            if req['number'] == req_no:
                try:
                    # Send to Burp's proxy history using sendToRepeater instead
                    message_info = req['messageInfo']
                    
                    # Send to Repeater tab (this will make it visible in proxy-like interface)
    ...
    def _update_display(self):
        ...
        # Track seen URLs and their first occurrence number
        seen_urls = {}
        ...
        for req in self.requests:
            ...
            screen_url = req['screen_url']
            display_number = req['number']
            
            if screen_url in seen_urls:
                # This is a duplicate URL, mark it with asterisk
                first_occurrence = seen_urls[screen_url]
                display_number = "{}*".format(first_occurrence)
            else:
                # First time seeing this URL, store the original number
                seen_urls[screen_url] = req['number']
            ...
            self.log_model.addRow([
                display_number,  # Use modified number with asterisk for duplicates
                req.get('screen_name', ""),
                screen_url,
                req.get('button_name', ""),
                req['method'],
    ...
    def getUiComponent(self):
        ...
        control_panel.add(self.export_button)  # Changed from copy_button
